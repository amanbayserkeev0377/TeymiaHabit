...
struct OverviewHeatmapView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ...
                // Year stats (как у привычек - сверху справа)
            ...
            .padding(.horizontal, 16) // Как у привычек
            
            // Heatmap Grid с осями - ЕДИНЫЙ ScrollView (как в HabitHeatmapCard)
            ...
            .padding(.horizontal, 8) // Как у привычек
            
            // Legend (GitHub-style) - справа как у привычек
            HStack {
                Spacer()
    ...
    private func generateOverallHeatmapData() {
        ...
        while currentDate <= min(endOfYear, Date()) {
            let completionRate = calculateOverallCompletionRate(for: currentDate)
    ...
    private func calculateOverallCompletionRate(for date: Date) -> Double {
        ...
        // Calculate AVERAGE completion rate across all active habits (improved!)
        ...
        for habit in activeHabits {
            ...
            if goal > 0 {
                let rate = min(Double(progress) / Double(goal), 1.0) // Cap at 100%
                totalProgress += rate
                totalGoals += 1.0
            }
        }
    ...
    private func colorForCompletionRate(_ rate: Double) -> Color {
        let baseColor = AppColorManager.shared.selectedColor.color
        
        // GitHub-style 5-level intensity system (с цветом приложения)
        switch rate {
        case 0:
            // No activity - light gray
            return Color.gray.opacity(0.1)
        case 0.01..<0.25:
            // Low activity (1-24%) - very light app color
            return baseColor.opacity(0.3)
        case 0.25..<0.50:
            // Medium-low activity (25-49%) - light app color
            return baseColor.opacity(0.5)
        case 0.50..<0.75:
            // Medium activity (50-74%) - medium app color
            return baseColor.opacity(0.7)
        case 0.75..<1.0:
            // High activity (75-99%) - strong app color
            return baseColor.opacity(0.9)
        default:
            // Complete (100%) - full app color
            return baseColor
        }
    }
