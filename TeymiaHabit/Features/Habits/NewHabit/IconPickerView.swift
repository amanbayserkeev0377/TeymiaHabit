...
struct IconPickerView: View {
    ...
    private var filteredIcons: [CategorySection] {
        let query = searchText.lowercased().trimmingCharacters(in: .whitespaces)
        if query.isEmpty { return categories }
        
        return categories.compactMap { category in
            let matchingIcons = category.icons.filter { $0.lowercased().contains(query) }
            let categoryMatches = category.name.lowercased().contains(query)
            
            if categoryMatches {
                return category
            } else if !matchingIcons.isEmpty {
                return CategorySection(name: category.name, icons: matchingIcons)
            }
            return nil
        }
    }
