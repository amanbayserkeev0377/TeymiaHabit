    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ChartPeriodHeader(
                ...
                onPrevious: { withAnimation(.easeInOut(duration: 0.3)) { currentWeekIndex -= 1 } },
                onNext: { withAnimation(.easeInOut(duration: 0.3)) { currentWeekIndex += 1 } }
            )

            ChartContainer(currentIndex: $currentWeekIndex, count: weeks.count) {
                chartView
            }
